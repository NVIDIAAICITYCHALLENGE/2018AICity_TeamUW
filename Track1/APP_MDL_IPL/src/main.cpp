...
float compAppMdl(char *acInFeatFlrPthPrb, int nIdPrb, char *acInFeatFlrPthGall, int nIdGall, int nHistCmpTyp, int nEmdDistTyp = CV_DIST_L1)
{
	...
	std::sprintf(acInFeatPth, acInFeatFlrPthPrb);
	...
	std::sprintf(acInFeatPth, acInFeatFlrPthGall);
	std::strcat(acInFeatPth, acInFeatNm);
	ifsFeat.open(acInFeatPth);
	ifsFeat.getline(acInFeatBuf, 2048);
	while (!ifsFeat.eof())
...
{
	char acInTrkTxt[256] = {};
	std::sprintf(acInTrkTxt, argv[1]);
	char acInFrmFlrPth[256] = {};
	std::sprintf(acInFrmFlrPth, argv[2]);
	char acOutFlrPth[256] = {};
	std::sprintf(acOutFlrPth, argv[3]);
			...
			for (std::vector<CTrkNd>::iterator it = voTrkNd.begin(); it != voTrkNd.end(); ++it)
			{
				...
				if (!it->getFrmBrdrFlg())
				{
					...
					if (APP_MDL_VIS_THLD < ((double)nMskArea / (double)nMskInitArea))
					{
						...
						std::sprintf(acOutFeatPth, acOutFlrPth);
